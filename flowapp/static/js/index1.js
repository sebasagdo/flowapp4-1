...
var paper = new Raphael(document.getElementById('canvas_container'), 600, 525);  
  ...
  var height = 60, x = 170, scale=20, devices = oData.devices.length;
  var style = {
    bg: 'rgba(0,0,0,.4)',
  ...
  for(var i=0; i<100; i++) {
    var xx = x+(i*scale);
    var line = paper.path('M '+xx+',15 L '+xx+','+(height*devices*1.1155));
  ...
  for(var i=0; i<devices; i++) {
    ...
    rectangle.animate({"width": width}, 100*me.ratio/30);
